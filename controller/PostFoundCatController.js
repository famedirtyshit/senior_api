...
const sendEmailIdle = async (req, res, next) => {
        try {
                ...
                if (!payload.postId || !payload.credential) {
                        ...
                        let allPost;
                        let postResult = await postFoundCatModel.find({ _id: postIdObjectIdFormat }).populate('owner').exec();
                        allPost = postResult;
                        if (allPost.length > 0) {
                                                ...
                                                .then((userRecord) => {
                                                        ...
                                                        if (post.owner.mailSubscribe == true) {
                                                                transporter.sendMail({
                                                                        ...
                                                                        subject: "แจ้งเตือนหมดเวลาโพสต์พบแมวของคุณ",                      // หัวข้อ
                                                                        html: `<p><b>โพสต์พบเจอแมวของคุณจะหมดเวลาในวันที่ ${inactiveDueDate.format('DD/MM/YYYY')}</b></p><br>
                                                                        <p>พบวันที่: ${postDateSelected.format('DD/MM/YYYY')}</p><br>
                                                ...
                                                .then((userRecord) => {
                                                        ...
                                                        if (post.owner.mailSubscribe == true) {
                                                                transporter.sendMail({
                                                                        from: process.env.CATUS_MAIL_USER,   // ผู้ส่ง
                                                                        to: userRecord.toJSON().email,// ผู้รับ
                                                                        subject: "แจ้งเตือนโพสต์พบแมวของคุณอยู่ในสถานะไม่ได้ใช้งาน",                      // หัวข้อ
                                                                        html: `<p><b>โพสต์พบเจอแมวของคุณอยู่ในสถานะไม่ได้ใช้งาน หากต้องการประกาศโพสต์นี้อีกครั้งกรุณาดำเนินการที่เว็บไซต์ <span><a href="https://dev-next-cloud-run-4p3fhebxra-as.a.run.app">CatUs</a></span> โดยโพสนี้จะหมดอายุในวันที่ ${inactiveDueDate.format('DD/MM/YYYY')}</b></p><br>
                                                                        <p>พบวันที่: ${postDateSelected.format('DD/MM/YYYY')}</p><br>
                                                                        <p>เพศ: ${post.sex == 'unknow' ? 'ไม่ทราบ' : post.sex == 'true' ? 'ตัวผู้' : 'ตัวเมีย'}</p><br>
                                                                        <p>ปลอกคอ: ${post.collar == true ? 'มีปลอกคอ' : 'ไม่มีปลอกคอ'}</p><br>
                                                                        <p>คำอธิบายเพิ่มเติม: ${post.description ? post.description : '-'}</p><br><br>
        ...
        try {
                ...
                if (!payload.postId || !payload.credential) {
                        ...
                        if (allPost.length > 0) {
                                ...
                                allPost.map((post, index) => {
                                        bucket.deleteFiles({
                                                prefix: `found/${post._id.toString()}`
                                        }, function (err) {
                                                if (err) {
                                                        e = new Error(err.body);
                                                        e.message = err.message;
                                                ...
                                                .then((userRecord) => {
                                                        ...
                                                        if (post.owner.mailSubscribe == true) {
                                                                transporter.sendMail({
                                                                        from: process.env.CATUS_MAIL_USER,   // ผู้ส่ง
                                                                        to: userRecord.toJSON().email,// ผู้รับ
                                                                        subject: "แจ้งเตือนโพสต์พบแมวของคุณหมดอายุ",                      // หัวข้อ
                                                                        html: `<p><b>โพสต์พบแมวของคุณหมดอายุแล้ว</b></p><br>
                                                                        <p>พบวันที่: ${postDateSelected.format('DD/MM/YYYY')}</p><br>
                                                                        <p>เพศ: ${post.sex == 'unknow' ? 'ไม่ทราบ' : post.sex == 'true' ? 'ตัวผู้' : 'ตัวเมีย'}</p><br>
                                                                        <p>ปลอกคอ: ${post.collar == true ? 'มีปลอกคอ' : 'ไม่มีปลอกคอ'}</p><br>
                                                                        <p>คำอธิบายเพิ่มเติม: ${post.description ? post.description : '-'}</p><br><br>`,
...
const extendPost = async (req, res, next) => {
        try {
                ...
                const originalCredential = bytes.toString(CryptoJS.enc.Utf8);
                connectDB();
                let postTarget = await postFoundCatModel.findById({ _id: mongoose.Types.ObjectId(payload.postId) }).populate('owner').exec();
                if (postTarget.owner._id != originalCredential) {
                        res.status(403).json({ result: false, msg: 'you don\'t have access' })
                        return;
                }
                if (postTarget.status == 'active') {
                        let newDate = new Date();
                        let updateRes = await postFoundCatModel.findOneAndUpdate({ _id: mongoose.Types.ObjectId(payload.postId) }, { createdAt: newDate, status: 'active', idle: false }, { new: true, timestamps: false }).exec();
                        res.status(200).json({ result: true, updateResult: updateRes });
                } else if (postTarget.status == 'inactive') {
                        let newDate = new Date();
                        let updateRes = await postFoundCatModel.findOneAndUpdate({ _id: mongoose.Types.ObjectId(payload.postId) }, { createdAt: newDate, status: 'active', idle: false }, { new: true, timestamps: false }).exec();
                        ...
                        query.where('location').equals({
                                $near: {
                                        ...
                                        $geometry: {
                                                type: "Point",
                                                coordinates: [updateRes.location.coordinates[0], updateRes.location.coordinates[1]]
                                        }
                                }
                        }).exec().then(nearRes => {
                                let lostPostInArea = [];
                                nearRes.map(post => {
                                        if (post.owner._id.toString() != updateRes.owner._id.toString()) {
                                                lostPostInArea.push(post._id);
                                        }
                                })
                                postLostCatModel.updateMany({ _id: lostPostInArea }, { $push: { nearFoundCat: { _id: mongoose.Types.ObjectId(updateRes._id) } } }, null, (err, result) => {
                                        if (err) {
                                                e = new Error(err.body);
                                                e.message = err.message;
                                                e.statusCode = err.statusCode;
                                                ...
                                                res.status(200).json({ result: true, updateResult: updateRes });
                                                nearRes.map(lostPost => {
                                                        if (lostPost.owner._id.toString() != updateRes.owner._id.toString()) {
                                                                let session = sessionMap.get(lostPost.owner.toString());
                                                                if (session != undefined && session.length > 0) {
                                                                        session.map(item => {
                                                                                io.to(item).emit('newNearPost', { foundPost: updateRes, lostPost: lostPost })
                                                                        })
                                                                }
                                                        }
                                                })
